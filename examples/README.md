...
Twenty-five steps run with `FAUCET_URL` set, twenty-two without. Steps 9 to 11
are the faucet steps, and the numbers below assume they run. Every step drives
the API that `TOKEN_STANDARD_VERSION` selects, and prints the path it took.

**Step 1: Check sender balance**
- **Given** the sender party is configured with valid Keycloak credentials
- **When** fetching the sender's holdings of the instrument
...
- **When** fetching the receiver's holdings of the instrument
...
**Step 4: Accept free credential offer (sender)** *(only if `RUN_CREDENTIAL_ACCEPT=1`)*
- **Given** the sender has a pending credential offer
- **When** calling `accept_credential_offer` on the first offer
- **Then** a credential contract is created. Off by default, because the
  contract has no archive choice and would accumulate across runs.

**Step 5: Fetch account rules**
...
**Step 6: Create deposit account**
- **Given** credential CIDs from step 3 and account rules from step 5
...
- **Given** the deposit account from step 6
- **When** calling `get_bitcoin_address` via the Bitsafe API
- **Then** a valid BTC address is returned for the account

**Step 8: Create withdraw account**
- **Given** credential CIDs from step 3, withdraw account rules from step 5, and a destination BTC address
...
**Steps 9-11: Faucet deposit** *(only if `FAUCET_URL` is set)*
...
- **Given** a faucet request submitted in step 9
...
- **When** submitting the same transfer again
- **Then** a transfer offer is created, for the steps below to accept

**Step 15: List outgoing offers (sender)**
- **Given** a transfer was submitted in step 14
- **When** fetching the sender's outgoing transfer offers for the instrument
- **Then** at least one pending offer exists

**Step 16: List incoming offers (receiver)**
- **Given** a transfer offer exists from step 14
- **When** fetching the receiver's incoming transfer offers for the instrument
- **Then** at least one pending offer exists

**Step 17: Accept transfers (receiver)**
...
**Step 18: Check receiver balance**
- **Given** the receiver accepted a transfer in step 17
- **When** fetching the receiver's balance
- **Then** the balance reflects the received amount

**Step 19: Return CBTC to sender**
- **Given** the receiver has CBTC from step 17
...
**Step 20: Accept transfers (sender)**
- **Given** the sender has a pending incoming offer from step 19
- **When** calling `accept_all` for the sender party
- **Then** all offers are accepted with zero failures

**Step 21: Check sender balance (pre-withdraw)**
- **Given** the sender accepted the return transfer in step 20
- **When** fetching the sender's balance
- **Then** the balance is captured as the pre-withdraw baseline

**Step 22: Submit withdrawal**
- **Given** the withdraw account from step 8 and the sender has CBTC holdings
...
**Step 23: Check balance (post-withdraw)**
- **Given** the sender's pre-withdraw balance from step 21
...
- **When** splitting it into one output worth half its value
- **Then** the split reports its outputs and its change UTXOs, and a failure
  reports the holdings it did create
